...
def main(args):
    ...
    logdir2 = f"../../logs/exp064_finetune/biased_conv_transformer_192_12_32_7_ALiBi_swiGLU_ep300_lr1e_3_bs256/fold{args.fold}/"
    filename2 = logdir2 + "test_pseudo_label"
    pl_path2 = sorted(glob(f"{filename2}_*"))
    df_filter_2A3 = []
    df_filter_DMS = []
    df_all_len = 0
    filtered_len = 0
    for p, p2 in tzip(pl_path, pl_path2):
        df = pd.read_parquet(p)
        df2 = pd.read_parquet(p2)
        for key in df:
            if "reactivity" in key:
                df[key] = (df[key] + df2[key]) / 2
        df_2A3 = df.loc[
