...
test_bpp_paths_contrafold = glob(f"{SEQ_PATH}/bp_matrix_contrafold/test*/**/*.npy", recursive=True)
test_id_to_bpp_paths_contrafold = {p.split("/")[-1].split(".")[0]: p for p in test_bpp_paths}
...
class RNA_Dataset_Test(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        bp_matrix_contrafold = np.load(test_id_to_bpp_paths_contrafold[seq_id])  # Add
...
def main(args):
    ...
    for fold in range(5):
        ...
        df.to_parquet(
            f"../../logs/{args.logdir}/fold{fold}/submission.parquet",
            index=False,
        )  # 6.5GB
        print(df.head())
