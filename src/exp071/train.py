...
COMMENT = """
    ...
    param tuning from exp034, GeGLU, split attention ALiBi and bpps, 
    fixed 0-1 clipping, B2T connection option, low grad clipping,
    add error target, stage1 for pseudo label,
    add norm and act for conv1d, GeGLU, use contrafold
    """
...
train_bpp_paths_contrafold = glob(
    f"{SEQ_PATH}/bp_matrix_contrafold/train*/**/*.npy", recursive=True
)
train_id_to_bpp_paths_contrafold = {
    p.split("/")[-1].split(".")[0]: p for p in train_bpp_paths
}
...
class RibonanzaDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        bp_matrix_contrafold = np.pad(
            ...
            (
                (0, self.Lmax - len(bp_matrix_contrafold)),
                (0, self.Lmax - len(bp_matrix_contrafold)),
            ),
        )  # Add
...
class BiasedConvTransformerEncoderLayer(nn.TransformerEncoderLayer):
    def __init__(
        ...
        self.linear1 = nn.Linear(d_model, dim_feedforward * 2)  # for GeGLU
        ...
        if self.norm_first:
            if self.use_second_bias:
                x_att, bias_att = self._sa_block(
                    self.norm1(x), bias_second, src_mask, src_key_padding_mask
                )
            else:
                x_att, bias_att = self._sa_block(
            ...
            bias = bias + bias_att
            bias_second = bias_second + bias_att
        else:
            if self.use_second_bias:
                x_att, bias_att = self._sa_block(
                    x, bias_second, src_mask, src_key_padding_mask
                )
            else:
                x_att, bias_att = self._sa_block(
                    x, bias, src_mask, src_key_padding_mask
                )
...
class RibonanzaModel(nn.Module):
        ...
        for i in range(depth):
            transformer_encoder.append(
                BiasedConvTransformerEncoderLayer(
                    ...
                    use_second_bias=False if i % 2 == 0 else True,
                )
            )
        self.transformer_encoder = nn.ModuleList(transformer_encoder)
        self.proj_out = nn.Linear(dim, 4)
    ...
    def forward(self, x0):
        ...
        bias_bpps_contrafold = x0["bp_matrix_contrafold"][:, None, :Lmax, :Lmax]
        x = self.seq_emb(x_seq)
        for i in range(len(self.transformer_encoder)):
            x, bias_bpps, bias_bpps_contrafold = self.transformer_encoder[i](
                x,
                bias_bpps,
                bias_second=bias_bpps_contrafold,
